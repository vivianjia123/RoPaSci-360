...
def offense_route_opt(my_token_list, goal, prev_pos, my_token_array, opponent_token_array, adjacent_token_list):
    '''
    Generate a list of player's actions based on offense purpose.

    :param my_token_list: a list contain all tokens of player
    :param goal: the coordinate (r, q) of closest goal
    :param opponent_token_array: the opponent tokens list
    :param prev_pos: the coordinate that current token comes from
    :param adjacent_token_list: the list contains all adjacent tokens
    :param my_token_array: the player's tokens list
    :return: a list contain all offense options of the player 
    '''
...
def defense_opt(my_token_list, opponent_token_array, adjacent_token_list, my_token_array): 
    '''
    Generate a list of player's actions based on defense purpose.

    :param my_token_list: a list contain all tokens of player
    :param opponent_token_array: the opponent tokens list
    :param adjacent_token_list: the list contains all adjacent tokens
    :param my_token_array: the player's tokens list
    :return: a list contain all defense options of the player
    '''
    explored_pos_lst = explored_pos_lst = [my_token_list[1]]
    token_pos = my_token_list[1]
    token_type = my_token_list[0]
    # threat_list is the list of all threats while threat_token_list is the info of a single threat
    ...
    if not any (escape_opts_is_covered):
        if not run_for_cover(my_token_list, opponent_token_array, adjacent_token_list, my_token_array):
            ...
            if potential_move and gen_closest_goal(potential_move, threat_list):
...
def evasive_attack(my_token_list, opponent_token_array, adjacent_token_list, my_token_array):
    explored_pos_lst = explored_pos_lst = [my_token_list[1]]
    token_pos = my_token_list[1]
    token_type = my_token_list[0]
    threat_list = my_token_list[3]
    escape_opts = gen_possible_moves(my_token_list[1], my_token_list[0], opponent_token_array, adjacent_token_list, my_token_array, explored_pos_lst, show_text=False)
    escape_opts_copy = escape_opts.copy()
    if my_token_list[3]:
        closest_threat = gen_closest_goal(my_token_list[1], my_token_list[3])
        for move in escape_opts_copy:
            if calculate_distance(move, closest_threat) == 1:
                escape_opts.remove(move)
        #print("**DEBUG evasive_attack", my_token_list)
        #print("**DEBUG escape_opts", escape_opts)
        #print("**DEBUG escape_opts_copy", escape_opts_copy)
        if my_token_list[2]:
            closest_goal = gen_closest_goal(my_token_list[1], my_token_list[2])
            if escape_opts:
                closest_move_to_goal = escape_opts[0]
                closest_dis_to_goal = calculate_distance(escape_opts[0], closest_goal)
                for move in escape_opts:
                    if calculate_distance(move, closest_goal) < closest_dis_to_goal:
                        closest_dis_to_goal = calculate_distance(move, closest_goal)
                        closest_move_to_goal = move
                return ("Move", my_token_list[1], closest_move_to_goal)
...
def throw_action(throw_range, player, throws_left, opponent_token_array, my_token_array):
    ...
    if throws_left <= 0: # No throws left
        ...
        if (len(my_token_array) == 0) and (len(opponent_token_array) == 0): # No friendly or hostile tokens on board yet
                        ...
                        for token in my_token_array:
                            if token[1] == throw_dest:
                                if token[0] == counter_token_type:
                                    ...
                                    if throw_dest in available_throw_contacts:
            ...
            if (len(throw_choices[0]) != 0)  or (len(throw_choices[1]) != 0):
                pass
            else:
                if type_choice and (len(my_token_array) < 3): # Perform a protective throw if number of tokens < than 1p 1r 1s
        ...
        elif (len(my_token_array) != 0) and (len(opponent_token_array) == 0): # Throw is unnecessary
            return False
        return throw_choices

def covering_move(my_token_array):
    # Warning: Might result in friendly kill
    action_list = []
    for token_1 in my_token_array:
        adjacency_list = get_token_adjacency(token_1[1], my_token_array)
        for token_2 in my_token_array:
            if token_2 in adjacency_list:
                if token_1[0] == protective_type(token_2):
                    action_list.append(("Move",token_1[1], token_2[1]))
    return action_list

def offensive_throw(enemy_token_array, player, my_throw_range):
    # Used only when opponent throws = 0 and my throws > 0 
    if player == "upper":
    ...
    type_list = []
    for enemy in enemy_token_array:
        if counter_type(enemy[0]) not in type_list:
            type_list.append(counter_type(enemy[0]))

    action_list = []
    for types in type_list:
        for coord in allowed_coordinates:
            action_list.append(("THROW", types, coord))
    return action_list

def covering_throw(my_token_array, enemy_token_array, faction, my_throw_range, my_throws_left, enemy_throw_range, enemy_throws_left): 
    # Warning: Might result in friendly kill
    if my_throws_left <= 0:
        return     
    if faction == "upper":
        allowed_r_coord = [i for i in range(4 - my_throw_range, 5)]
        op_allowed_r_coord = [i for i in range(-4 , -3 + enemy_throw_range)]
    else:
        allowed_r_coord = [i for i in range(-4 , -3 + my_throw_range)]
        op_allowed_r_coord = [i for i in range(4 - enemy_throw_range, 5)]
    allowed_q_coordinate = [i for i in range(-4 , 5)]
    raw_coordinates = []
    op_raw_coordinates = []
    for r in allowed_r_coord:
        for q in allowed_q_coordinate:
            raw_coordinates.append((r,q))
    for r in op_allowed_r_coord:
        for q in allowed_q_coordinate:
            op_raw_coordinates.append((r,q))
    allowed_coordinates = raw_coordinates.copy()
    op_allowed_coordinates = op_raw_coordinates.copy()
    for coordinate in raw_coordinates:
        if not boarder_check(coordinate):
            allowed_coordinates.remove(coordinate)
    for coordinate in op_raw_coordinates:
        if not boarder_check(coordinate):
            op_allowed_coordinates.remove(coordinate)

    action_list = []
    for token_1 in my_token_array:
        if (token_1[1] in allowed_coordinates) and (token_1[1] in op_allowed_coordinates):
            if enemy_throws_left > 0:
                action_list.append(("THROW", protective_type(token_1[0]), token_1[1]))
        else:
            if token_1[1] in allowed_coordinates:
                for enemy in enemy_token_array:
                    adjacency_list = get_token_adjacency(enemy[1], enemy_token_array)
                    explored_lst = [enemy[1]]
                    possible_moves = gen_possible_moves(enemy[1],enemy[0], my_token_array, adjacency_list, enemy_token_array, explored_lst)
                    if (token_1[1] in possible_moves) and (enemy[0] == counter_type(token_1[0])):
                        action_list.append(("THROW", protective_type(token_1[0]), token_1[1]))
